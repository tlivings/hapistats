...
exports = module.exports = {
    ...
    create : function (metrics) {
        ...
        if (cluster.workers.length > 0) {
            if (cluster.isMaster) {
                broker = service0.broker({ address: 'ipc://metrics', broadcast: 'ipc://workers' });
            }
            else {
                service0.service(track).connect('ipc://workers');
            }
            return broker;
        }
        else {
            if (cluster.isMaster) {
                return service0.service(track).bind('ipc://metrics');
            }
        }
